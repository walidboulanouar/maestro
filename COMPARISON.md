...
|---|:--:|:--:|:--:|
| Runs with **no GPU / no weights** | ✅ `npx openmaestro serve` + your API key | ❌ needs Python + GPU + Qwen3-0.6B weights | ❌ needs GPU + SGLang + LoRA adapters |
...
1. **It actually runs without a GPU, for anyone.** No GPU, no weights, no Python: `npx openmaestro serve` with one API key works in about 30 seconds. OpenFugu and MoL both require a GPU box and model artifacts.
