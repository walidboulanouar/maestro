...
|---|:--:|:--:|:--:|
| Runs with **no GPU / no weights** | ✅ `npm run serve` + your API key | ❌ needs Python + GPU + Qwen3-0.6B weights | ❌ needs GPU + SGLang + LoRA adapters |
...
| **Tests / CI** | ✅ 72 tests + CI | ⚠️ partial; training code **stripped (~78% reconstructed)** | ❌ none shipped |
...
1. **It actually runs without a GPU, for anyone.** No GPU, no weights, no Python: `npm run serve` with one API key works in about 30 seconds. OpenFugu and MoL both require a GPU box and model artifacts.
...
- **OpenFugu reproduces the actual learned router** (frozen Qwen3-0.6B + trained head, the papers' real mechanism). Maestro v0 uses a *heuristic* classifier — a learned (GPU) router is intentionally out of scope (Maestro stays no-GPU). If you want to study Fugu's ML internals, OpenFugu is the reference (and we credit it).
