...
>
> - Runs from a clone with one provider key: `npm run serve`
...
`npm run serve` (after a git clone + npm install)
...
3. Open `test.html` while `npm run serve` runs → click **Run all tests**.
