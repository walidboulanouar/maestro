...
- ✅ **You CAN say:** "It reaches frontier-level results at ~1/11th the cost on our routing benchmark (94% of best-single quality, 91% cheaper)" — because `npm run eval` reproduces it.
...
- **Key insight:** the **verifier must judge independently of the router**, or the loop never escalates. Sharing the signal scored *below random* (63%); splitting them → 94%. (Documented in VERIFICATION.md.)
